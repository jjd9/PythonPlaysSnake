...
class GridWorld:
    ...
    def update_state(self):
        rad = 5
        board = np.full((self.cols+2*rad, self.rows+2*rad),OBS_ID).astype("float")
        board[rad:self.cols+rad,rad:self.rows+rad] = EMPTY_ID
        board[self.food[0]+rad, self.food[1]+rad] = FOOD_ID
        board[self.snake.body[:,0]+rad, self.snake.body[:,1]+rad] = SNAKE_ID
        head_loc = self.snake.body[0,:]+rad
        _up = board[head_loc[0]-rad:head_loc[0], head_loc[1]]
        _down = board[head_loc[0]:head_loc[0]+rad, head_loc[1]]
        _left = board[head_loc[0], head_loc[1]-rad:head_loc[1]]
        _right = board[head_loc[0], head_loc[1]:head_loc[1]+rad]
        dist = np.sign(self.food - self.snake.body[0,:]).astype("float")
        self.state = np.hstack((dist, _up, _down, _left, _right))
