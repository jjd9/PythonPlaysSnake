...
class GridWorld:
    ...
    def update_state(self, snake_collision=False):
        if self.grid_state:
            self.state = np.zeros((self.rows, self.cols,3), np.uint8)
            self.state[self.food[0], self.food[1],:] = RED
            self.state[self.snake.body[1:,0], self.snake.body[1:,1],:] = GREEN
            self.state[self.snake.body[0,0], self.snake.body[0,1],:] = BLUE
            self.state = cv2.flip(self.state,1)
            self.state = np.rot90(self.state,1).reshape(3,self.rows,self.cols)
        else:
            self.board[:] = EMPTY_ID
            self.board[self.food[0], self.food[1]] = FOOD_ID
            self.board[self.snake.body[1:,0], self.snake.body[1:,1]] = SNAKE_ID
            self.board[self.snake.body[0,0], self.snake.body[0,1]] = HEAD_ID
            ...
            if self.snake.body.shape[0] > 1:
                if snake_head[0]+1 <=  self.cols-1:             
                    self.state[2] =  self.board[snake_head[0]+1,snake_head[1]] #right

                if 0<snake_head[0]:
                    self.state[3] =  self.board[snake_head[0]-1,snake_head[1]] # left

                if snake_head[1]+1 <= self.rows-1:
                    self.state[4] =  self.board[snake_head[0],snake_head[1]+1] #  down

                if 0 < snake_head[1]:
                    self.state[5] =  self.board[snake_head[0],snake_head[1]-1] # up
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            a = env.state
            if flip:
                a = np.flip(a.reshape(20,20,3), 1).reshape(3,20,20)
            a = np.rot90(a.reshape(20,20,3), rotation)
            cv2.imshow("main",a)
            # w, a, s, d
            # 101, 115, 100, 102
            val = cv2.waitKeyEx()
